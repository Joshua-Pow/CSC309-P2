...
class ContactViewSet(viewsets.ModelViewSet):
    ...
    def add(self, request):
        ...
        contact_exists = Contact.objects.filter(Q(userA=userA, userB=userB)|Q(userA=userB, userB=userA)).first()
        if contact_exists:
    ...
    def block(self, request):
        ...
        userBlookup = get_object_or_404(User, username=userB_username)
        contact = Contact.objects.filter((Q(userA=userBlookup, userB=request.user)|Q(userA=request.user, userB=userBlookup))).first() # will only be one but errors if not a check to make sure its 1
        if not contact: #no relationship - make new one thats blocked (searching + blocking)
    ...
    def unadd(self, request):
        ...
        contact = Contact.objects.filter((Q(userA=userBlookup, userB=request.user)|Q(userA=request.user, userB=userBlookup))).first() # will only be one but errors if not a check to make sure its 1
        if not contact:
            return Response({"error": "No existing relationship with user."}, status=status.HTTP_400_BAD_REQUEST)
